...
const NavBar = () => {
  ...
  return (
    <div className="relative top-0 z-50 flex items-center justify-between px-4 sm:px-6 py-4 shadow-lg border-gray-200 rounded-xl font-medium transition-all duration-500 ease-in-out">
      {/* Logo Section (Left) */}
      <Link to={token ? "/latest-inquiries" : "/login"} className="flex-shrink-0">
        <div className="w-[221px] h-[111px] bg-gradient-to-br from-purple-300 to-purple-100 rounded-2xl flex items-center justify-center p-2 shadow-xl hover:shadow-purple-400/60 transition-transform duration-500 hover:rotate-1 hover:scale-105">
          <img
            src={assets.logo}
            alt="Logo"
            className="w-full h-full object-contain transition-transform duration-500 ease-in-out"
          />
          <div className="absolute top-0 -left-4 w-72 h-72 bg-purple-300 rounded-full mix-blend-overlay filter blur-3xl pointer-events-none opacity-40 animate-blob z-0" />
          <div className="absolute top-0 -right-4 w-72 h-72 bg-yellow-500 rounded-full mix-blend-overlay filter blur-3xl pointer-events-none opacity-40 animate-blob animation-delay-2000 z-0" />
          <div className="absolute bottom-0 left-20 w-72 h-72 bg-pink-500 rounded-full mix-blend-overlay filter blur-3xl pointer-events-none opacity-40 animate-blob animation-delay-4000 z-0" />
        </div>
      </Link>
