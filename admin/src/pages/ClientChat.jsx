...
const ClientChat = () => {
  ...
  return (
    <div className="min-h-screen rounded-4xl font-orbitron p-4 relative overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto rounded-3xl border shadow-glow-fuchsia transition-all duration-700 ease-in-out hover:shadow-glow-purple">
        <div className="flex h-[calc(100vh-2rem)]">
          {/* Sidebar */}
          <div className="w-1/4 border-rshadow-inner-dark-glow scrollbar-thin scrollbar-thumb-fuchsia-800/50 scrollbar-track-transparent">
          ...
          {/* Chat Panel */}
          {selectedClient ? (
            <div className="flex-1 flex flex-col bg-transparent bg-gradient-to-b from-black/50 to-transparent">
              {/* Client Header */}
              <div className="p-4 border-b border-fuchsia-800/40 bg-gradient-to-b from-purple-900/50 to-black/50">
                <div className="flex justify-between items-start mb-4">
                  <h2 className="text-2xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-lime-300 to-cyan-300 drop-shadow-xl tracking-tight">
                    [CLIENT: {selectedClient.name.toUpperCase()}]
                  </h2>
                  <span className="px-5 py-2 bg-green-700/40 text-green-300 rounded-full text-md font-semibold shadow-inner-green">
                    STATUS:{" "}
                    {selectedClient?.projectDetails?.paymentStatus?.toUpperCase() ||
                      "PENDING"}
                  </span>
                </div>
                <div className="grid grid-cols-2 gap-15 text-sm text-gray-300">
                  <div className="space-y-2">
                    ...
                    <p>
                      <span className="font-bold text-white">[VALUATION]:</span>{" "}
                  ...
                  <div className="space-y-2">
                    ...
                    <p>
                      <span className="font-bold text-white">[INITIATED]:</span>{" "}
              ...
              <div
                ...
                className="flex-1 overflow-y-auto p-5 space-y-4 scrollbar-thin scrollbar-thumb-purple-700/60 scrollbar-track-transparent scrollbar-thumb-rounded-full"
              >
                {messages.map((message, idx) => (
                  ...
                  >
                    <div
                      className={`max-w-[75%] rounded-xl px-4 py-3 border relative shadow-lg transition-transform duration-300 ease-out hover:scale-[1.015] tracking-wide
        ${
          message.sender === "admin"
            ...
            : "bg-gradient-to-br from-black/30 via-slate-900/30 to-transparent border-cyan-500/20 text-cyan-100 animate-fade-in-left-glow shadow-glow-purple"
              ...
              {/* Input */}
              <div className="p-5 border-t border-fuchsia-800/40 bg-black/50">
                <form onSubmit={handleSend} className="flex gap-4">
                  <input
                    ...
                    className="flex-1 bg-black/40 border border-fuchsia-700/50 text-cyan-300 placeholder-gray-500 px-5 py-3 rounded-xl focus:outline-none focus:ring-3 focus:ring-cyan-500/70 text-lg font-mono caret-cyan-400"
                  />
                  <button
                    type="submit"
            ...
            <div className="flex-1 flex items-center justify-center text-gray-600 text-3xl font-light italic font-orbitron">
