    ...
    >
      ...
      <div className="relative z-10 max-w-7xl mx-auto text-center px-4">
        ...
        <div className="flex flex-col items-center gap-y-16 lg:gap-y-20">
          {[0, 4, 7].map((start, rowIndex) => {
            const sliceEnd = start + (rowIndex === 2 ? 2 : rowIndex === 1 ? 3 : 4);
            const gridCols = rowIndex === 2 ? "grid-cols-1 sm:grid-cols-2 max-w-3xl" : rowIndex === 1 ? "grid-cols-2 sm:grid-cols-3 max-w-5xl" : "grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 max-w-6xl";

            return (
              <div key={rowIndex} className={`grid ${gridCols} gap-x-6 sm:gap-x-12 lg:gap-x-16 gap-y-16 mx-auto`}>
                {services.slice(start, sliceEnd).map((service, index) => {
                  const i = start + index;
                  return (
                    <div
                      key={service._id}
                      ref={(el) => (orbRefs.current[i] = el)}
                      onClick={() => handleItemInteraction(i)}
                    ...
                    >
                      ...
                      <div className="absolute top-[calc(100%+0.75rem)] text-center w-full transition-all duration-500 ease-out-back opacity-0 group-hover:opacity-100 group-hover:top-[calc(100%+2rem)] hidden sm:block">
                        ...
                        <p className="text-gray-200 text-sm sm:text-xs mt-1 px-2 sm:px-3 leading-snug tracking-wide whitespace-normal break-words">
                          {service.description}
                        </p>
                      </div>
                      ...
                      {activeIndex === i && (
                        <div className="absolute top-[calc(100%+1rem)] bg-gradient-to-br from-white/15 to-white/5 backdrop-blur-xl p-4 rounded-xl w-60 sm:hidden text-white text-center shadow-3xl border border-white/40 z-20 animate-pop-in">
                          ...
                          <p className="text-sm sm:text-xs text-gray-200 leading-snug whitespace-normal break-words">
                            {service.description}
                          </p>
                        </div>
                      )}
